...
import { Play, ChevronLeft, ChevronRight, Clock, Calendar, X, ExternalLink, Share2 } from "lucide-react"
            ...
            <h2 className="relative mb-6 text-3xl font-bold text-gray-900 dark:text-white md:text-5xl lg:text-6xl">
              <span className="relative inline-block">
                ...
                <span className="relative ml-3 inline-block">
                  <span className="text-red-600 dark:text-red-500">sharing</span>
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: "100%" }}
                  ...
                  <Dialog>
                      ...
                      <div className="group relative aspect-video w-full cursor-pointer overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[2px] transition-all duration-300 group-hover:bg-black/40">
                          ...
                          >
                            ...
                            <span className="mt-4 font-medium text-white text-lg tracking-wide">Play Video</span>
                          </motion.div>
                        </div>
                    ...
                    <DialogContent className="max-w-[90vw] sm:max-w-[1000px] p-0 border-none bg-transparent">
                      <div className="rounded-2xl bg-white/95 dark:bg-gray-900/95 shadow-2xl backdrop-blur-xl overflow-hidden border border-gray-100 dark:border-gray-800">
                        ...
                        <DialogTitle className="sr-only">{activeVideo.talkTitle}</DialogTitle>
                          ...
                          <div className="mb-4 flex items-start justify-between">
                            <div>
                              ...
                              <div className="mt-1 flex items-center gap-1 text-red-600 dark:text-red-400">
                                <span className="text-sm font-medium">{activeVideo.name}</span>
                                <span className="mx-1.5 h-1 w-1 rounded-full bg-red-600/70 dark:bg-red-400/70"></span>
                                <span className="text-sm text-gray-500 dark:text-gray-400">{activeVideo.title}</span>
                              </div>
                            </div>
                ...
                <div className="md:col-span-5 lg:col-span-4 p-6 md:p-10 flex flex-col justify-center">
                  ...
                  >
                    <div className="inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur-sm px-3 py-1 mb-4">
                      ...
                      <span className="text-xs font-medium uppercase tracking-wider text-white">Featured Talk</span>
                    </div>
                    
                    <h3 className="mb-4 text-2xl md:text-3xl font-bold text-white leading-tight">{activeVideo.talkTitle}</h3>
                    
                    <div className="mb-4 flex items-center">
                      <div className="mr-4 h-10 w-10 overflow-hidden rounded-full border-2 border-red-500">
                        <Image
                          src={activeVideo.imageSrc || "/speakers/placeholder.jpg"}
                      ...
                      <div>
                        <div className="font-medium text-white">{activeVideo.name}</div>
                        <div className="text-sm text-gray-300">{activeVideo.title}</div>
                      </div>
                    </div>
                    
                    <p className="mb-6 text-gray-300 line-clamp-3">{activeVideo.talkSummary}</p>
                  </motion.div>
            ...
            >
              <div className="flex justify-center mb-8">
                <TabsList className="grid grid-flow-col auto-cols-auto gap-1 bg-gray-100/80 dark:bg-gray-800/80 backdrop-blur-sm p-1 rounded-full">
                  ...
                  <TabsTrigger value="technology" className="rounded-full px-4">Technology</TabsTrigger>
                  <TabsTrigger value="storytelling" className="rounded-full px-4">Storytelling</TabsTrigger>
                  <TabsTrigger value="design" className="rounded-full px-4">Design</TabsTrigger>
                  <TabsTrigger value="ideas" className="rounded-full px-4">Ideas</TabsTrigger>
                </TabsList>
              </div>
              ...
              <TabsContent value="technology" className="mt-0">
                <VideoGrid videos={videos.filter(v => v.category.toLowerCase() === 'technology')} />
              </TabsContent>
              
              <TabsContent value="storytelling" className="mt-0">
                <VideoGrid videos={videos.filter(v => v.category.toLowerCase() === 'storytelling')} />
              </TabsContent>
              
              <TabsContent value="design" className="mt-0">
                <VideoGrid videos={videos.filter(v => v.category.toLowerCase() === 'design')} />
              </TabsContent>
              
              <TabsContent value="ideas" className="mt-0">
                <VideoGrid videos={videos.filter(v => v.category.toLowerCase() === 'ideas')} />
              </TabsContent>
            </Tabs>
          </motion.div>
              ...
              <div className="mb-4 flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-1 text-red-600 dark:text-red-400">
                    ...
                    <span className="mx-1.5 h-1 w-1 rounded-full bg-red-600/70 dark:bg-red-400/70"></span>
                    <span className="text-sm text-gray-500 dark:text-gray-400">{video.title}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="rounded-full gap-1">
